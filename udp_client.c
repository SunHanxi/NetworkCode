...
int main(int argc, char* argv[])
{
	...
	//若程序启动时制定了主机名，则将其设定，否则使用默认的主机，即loaclhost
	if (argc > 1)
	{
		host = argv[1];
	}
	...
	//创建socket
	socket_descriptor = socket(PF_INET, SOCK_DGRAM, pointer_of_protocol->p_proto);
	...
	*/
	int send_ret = sendto(socket_descriptor, buffer, strlen(buffer), 0, (struct sockaddr*)&socket_address, sizeof(socket_address));
	if (send_ret < 0)
	{
		printf("Send failed!\n");
		exit(1);
